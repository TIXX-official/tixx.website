...
function filterByTab(events: EventListItemDto[], tab: TabValue): EventListItemDto[] {
  const now = new Date();
...
export function HostDetailContent({
  ...
  const filteredEvents = useMemo(() => filterByTab(allEvents, activeTab), [allEvents, activeTab]);
      ...
      <div className="lg:grid lg:grid-cols-[1.5fr_1fr] lg:gap-10 lg:items-start">
        <div className="lg:col-span-2">
          {isVenue && venueMedias.length > 0 && (
            <Carousel
              images={venueMedias.map((m) => ({ url: m.mediaUrl, alt: venue?.name }))}
            ...
            <div className="mt-5 flex flex-row">
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-2 flex flex-row items-center gap-1">
                  <Text as="span" variant="body1Regular" className="text-grayscale-0">
              ...
              <div className="relative ml-4 h-[70px] w-[70px] flex-shrink-0 self-center overflow-hidden rounded-full bg-grayscale-700">
                {host.imageUrl && (
                  <Image src={host.imageUrl} alt={host.name} fill sizes="70px" className="object-cover" />
                )}
              </div>
            </div>
            {isVenue && venue && (
          ...
          {isVenue && host.description && (
            <div className="px-4 lg:px-0">
              ...
              <ExpandableCard title={t.description} content={host.description} />
            </div>
          )}
