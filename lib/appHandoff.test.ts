...
function entry(overrides: Partial<InitialBrowserEntry> = {}): InitialBrowserEntry {
  return {
    pathname: '/events/123',
...
describe('resolveAutomaticHandoffTarget', () => {
  ...
  it('does not run after a Next.js client navigation', () => {
    expect(
      resolve({ entry: entry({ pathname: '/' }) }),
    ).toBeNull();
  });

  it.each(['reload', 'back_forward'])('does not run for %s navigation', (type) => {
    expect(
      resolve({ entry: entry({ navigationType: type }) }),
    ).toBeNull();
  });
